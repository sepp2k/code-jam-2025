...
from pyscript.web import when, Element
...
def custom_nav() -> Element:
    ...
    return nav(
        ...
        style="background-color: #333;color: white;padding: 0.5em 1em; box-shadow: rgba(60, 64, 67, 0.3) 0px 1px 2px 0px, rgba(60, 64, 67, 0.15) 0px 2px 6px 2px;",
    )
...
def custom_code_block(text: str, *more, **kwargs) -> Element:
...
""")
    ...
    if kwargs.get("language"):
        language_tip = span(kwargs["language"], style=language_style)
    language_tip = language_tip or span("text", style=language_style)

    make_copy = kwargs.get("copy_tip", True)
    copy_tip = a(
        span("📄 copy"), 
        href="#", onclick=f"copyToClipboard(`{text}`, () => {{ this.querySelector('span').textContent = '✔ copied'; setTimeout(() => this.querySelector('span').textContent = '📄 copy', 2000); }})",
    ...
    return div(
        ...
        style="""
position: relative;
border-bottom: 4px solid #0065d7;
padding: 0.5em;
